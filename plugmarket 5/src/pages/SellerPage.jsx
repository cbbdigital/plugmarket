        ...
        <div style={{ display: "flex", gap: 20, alignItems: "flex-start", flexWrap: "wrap" }}>
          ...
          <div style={{ flex: 1, minWidth: 200 }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={() => loggedIn ? navigate(`/messages?seller=${id}`) : navigate("/login")} style={{ padding: "8px 18px", borderRadius: 10, border: "none", background: GR, color: "#fff", fontSize: 12, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", gap: 6 }}>
                <MailIcon size={14} color="#fff"/> {loggedIn ? "Message" : "Log in to message"}
              </button>
              <button onClick={() => loggedIn ? setShowPhone(!showPhone) : navigate("/login")} style={{ padding: "8px 18px", borderRadius: 10, border: `1px solid ${t.bd}`, background: t.card, color: t.tx, fontSize: 12, fontWeight: 500, cursor: "pointer", display: "flex", alignItems: "center", gap: 6 }}>
                <PhoneIcon size={14} color={t.tx2}/> {!loggedIn ? "Log in to see phone" : showPhone && seller.phone ? seller.phone : "Show phone"}
              </button>
            </div>
